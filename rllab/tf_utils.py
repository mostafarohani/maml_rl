  ...
  for i, layer in enumerate(layer_defs):
    if isinstance(layer, list):
      sublayers, sublayer_names = layer_factory(
          layer, name_prefix='%s_%d' % (name_prefix, i), list_flatten=False)
...
def _make_queued_step(core, inputs, dones):
  initial_state = snt.nest.flatten(core.initial_state(shape_if_known(dones, 0)))
...
class StackedRNN(snt.AbstractModule):
  ...
  def __init__(self, cores, name=None):
    ...
    with self._enter_variable_scope():
      ...
      self._rnn_core = snt.DeepRNN(cores, skip_connections=False) if cores else None
  ...
  def _build(self, inputs, prev_hidden=None):
    if inputs.get_shape().ndims == 3:
      ...
      if len(self._cores) == 0:
        ...
        outputs, _ = tf.nn.dynamic_rnn(self._rnn_core, inputs, dtype=tf.float32)
...
class CausalConv1D(snt.RNNCore):
  ...
  def _build(self, x_in, h_in=None, z=None):
    ...
    if mode == 'conv':
      ...
      # rnn mode
      h_out = snt.nest.flatten(list(h_in) + [x_in])
      xx = tf.stack(h_out, axis=1)

      if len(h_out) == pad_size + 1:
      ...
      h_out = tf.split(xx[:, 1:], k - 1, axis=1)
...
class DenseBlock(snt.RNNCore):
  ...
  def __init__(self, params, name='DenseBlock'):
    ...
    with self._enter_variable_scope():
      self.layers, = layer_factory(params)
      self._cores = Struct(
...
class CausalAttention(snt.RNNCore):
  ...
  def _build(self, inputs, prev_hidden=None):
    ...
    if inputs.get_shape().ndims == 3:
      ...
      if self._last_timestep_only:
        ...
        def get_idx(t):
          return np.array(sorted([(0, j, 0, i) for j in range(t)
                                  for i in range(t) if i - self._buffer_size <= j and j < i]))
        ...
        read = tf.reshape(read, [batch_size, t, self._num_heads * self._vals_dim])
      ...
      key_seq = tf.reshape(key_seq, [batch_size, self._buffer_size, self._num_heads, d])
      val_seq = tf.reshape(val_seq, [batch_size, self._buffer_size, self._num_heads, D])
      query = tf.reshape(query, [batch_size, self._num_heads, d])
      ...
      curr_keys = tf.reshape(self._cores.keys(inputs_expanded), [batch_size, 1, self._num_heads, d])
      curr_vals = tf.reshape(self._cores.vals(inputs_expanded), [batch_size, 1, self._num_heads, D])
  ...
  @property
  def output_size(self):
    return self._val_dim * self._num_heads
  ...
  def initial_state(self, batch_size, **unused_kwargs):
    ...
    return (
        ...
        tf.zeros([batch_size, self._buffer_size, self._num_heads], tf.bool)
    )
